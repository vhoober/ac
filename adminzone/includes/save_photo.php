...
{
    if(!isset($_POST['galleryimg']) && $_POST['galleryimg'] != 1) {
            // Insert into product_photo
            $productID			= trim($_POST['productID']);



            $insertProductSQL = "INSERT INTO product_photo (product_id, date) VALUES ('$productID', now())";
            $prod_photo_id = $db->executeNonQuery($insertProductSQL, $connectionParameters);
            unset($_SESSION['gallery_id']);
            $_SESSION['productID'] = $productID;
            $_SESSION['prod_photo_id'] = $prod_photo_id;
    }
    else
    {
            // insert into gallery
            ...
            $insertGallery = "INSERT INTO gallery (photo_name, photo_desc, date) VALUES ('$photoName', '$photoDesc', now())";
            $gallery_id = $db->executeNonQuery($insertGallery, $connectionParameters);
            unset($_SESSION['prod_photo_id']);
            unset($_SESSION['productID']);
            $_SESSION['gallery_id'] = $gallery_id;
    }
}
?>
<form name="form" enctype="multipart/form-data" method="post" action="website.php?act=action&mod=saveuploadphoto">
<div align="center">
    <p>&nbsp;</p>
<p>&nbsp;</p>
<table  border="0" cellpadding="0" cellspacing="0" id="table8">
	...
	<input type="file" name="upload" size="30" style="font-family: Arial; font-size: 9pt" ></font></span></td>
								...
								<input type="submit" value="upload picture" name="Submit" tabindex="8">&nbsp; <?php if($_POST['Submit'] == "upload picture"){?><input type="button" value="finish" name="B4" onClick="window.location='website.php?act=action&mod=uploadphoto'" tabindex="8"><?php }?></td>
												</tr>
												</table></td>
				<td><?php
                                ...
                                if($_POST['Submit'] == "upload picture")
                                {
                                    ...
                                    // set variables
                                    if(isset($_SESSION['prod_photo_id']))
                                    {
                                    // Original picture
									...
									{
                                    // Original picture
                                    $dir_dest_orig = (isset($_GET['dir']) ? $_GET['dir'] : 'images/gallery/original/');
                                    $dir_pics_orig = (isset($_GET['pics']) ? $_GET['pics'] : $dir_dest_orig);

                                    // Thumbnail picture
                                    $dir_dest_thumb = (isset($_GET['dir']) ? $_GET['dir'] : 'images/gallery/thumbnail/');
                                    $dir_pics_thumb = (isset($_GET['pics']) ? $_GET['pics'] : $dir_dest_thumb);									
									}
                                    ...
                                    $handle = new Upload($_FILES['upload']);
                                        ...
                                        if ($handle->uploaded) {
                                            ...
                                            if(isset($_SESSION['prod_photo_id']))
                                            {
                                            	$handle->file_new_name_body = $_SESSION['productID']."_".$_SESSION['prod_photo_id'];
                                            }
                                            else
                                            {
                                            	$handle->file_new_name_body = $_SESSION['gallery_id'];
                                            }
                                            ...
                                            else
                                            {
                                                ...
                                                if ($handle->processed) {
													...
													if(isset($_SESSION['prod_photo_id']))
													{
														$updatePhoto = "UPDATE product_photo SET format = '".strtolower($file[1])."' WHERE product_id = '".$_SESSION['productID']."' AND product_photo_id = '".$_SESSION['prod_photo_id']."'";
													}
													else
													{
														$updatePhoto = "UPDATE gallery SET format = '".strtolower($file[1])."' WHERE gallery_id = '".$_SESSION['gallery_id']."'";
													}
							
                                                    $db->executeNonQuery($updatePhoto, $connectionParameters);
                                                } else {
                                                    // one error occured
                                                    echo '<fieldset>';
